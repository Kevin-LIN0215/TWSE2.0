...
export default function Home() {
    ...
    try {
      ...
      const merged = stockData.map((s) => {
        ...
        const stock = {
          ...
          pe: toNumber(s.pe || s.PE || s["本益比"]),
          pb: toNumber(s.pb || s.PB || s["股價淨值比"]),

          roe: toNumber(s.roe || s.ROE || s["ROE"] || s["股東權益報酬率"]),
  ...
  const filteredStocks = stocks.filter((stock) => {
    ...
    return (
      ...
      (pe === 0 || pe <= maxPE) &&
      (stock.roe === 0 || stock.roe >= minROE) &&
      (stock.epsGrowth === 0 || stock.epsGrowth >= minEPSGrowth) &&
      (stock.revenueGrowth === 0 ||
        stock.revenueGrowth >= minRevenueGrowth) &&
      (stock.foreignBuyDays === 0 ||
        stock.foreignBuyDays >= foreignBuyDays) &&
      (stock.trustBuyDays === 0 || stock.trustBuyDays >= trustBuyDays) &&
